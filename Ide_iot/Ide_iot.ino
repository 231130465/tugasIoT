...
// Wi-Fi credentials
const char* ssid = "mikroskil";          //nama Wi-Fi anda (SSID)
const char* password = "231130465";  //password Wi-Fi anda

// Firebase credentials
#define API_KEY "AIzaSyDMgjxaR1NMZYRwk8dcbRvZb8VYzrpKVy4"           //Found in Project Settings > General
#define DATABASE_URL "https://tugas-224cf-default-rtdb.firebaseio.com"   //From Realtime Database URL
#define USER_EMAIL "juandaharefa1@gmail.com"               //email user saat membuat database
...
void bacaDanKirimData() {
  ...
  // Persentase untuk mendapat persentase kelembaban (0% kering, 100% basah)
  ...
  if (Firebase.ready()) {
    ...
    if (Firebase.RTDB.setDouble(&fbdo, basePath + "/timestamp", timestamp)) {
      Serial.printf("  ✓ timestamp terkirim (Unix): %lu\n", timestamp);
    } else {
